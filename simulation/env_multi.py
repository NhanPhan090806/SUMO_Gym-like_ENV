...
import time
import random
import threading    # ← NEW: used only for the thread-safe instance ID counter
...
# =============================================================================
# TraCI is a global singleton: bare `traci.vehicle.getSpeed()` always operates
# on whichever SUMO process was most recently started.  Running multiple envs
# in the same process therefore requires each instance to own its own isolated
# connection object.
#
# How it works:
#   1.  Each SumoEnv gets a unique integer _env_id at construction time,
#       guarded by a threading lock so it is safe even if envs are created
#       from multiple threads.
#   2.  reset() calls traci.start(cmd, label=self._label, port=self._port)
#       which launches a *separate* SUMO process and registers the connection
#       under a unique name.
#   3.  traci.getConnection(self._label) returns a connection object whose
#       .vehicle / .lane / .simulation / ... sub-modules work exactly like the
#       global traci module but talk only to that one SUMO process.
#   4.  Every TraCI call in the class goes through self._traci instead of
#       the global traci module — that is the *only* change to the logic.
#
# DummyVectorEnv calls each env sequentially in a single thread, so there is
# no race condition — but the approach is also safe for SubprocVectorEnv.

_env_id_lock    = threading.Lock()
_env_id_counter = 0           # incremented once per SumoEnv.__init__ call
BASE_TRACI_PORT = 8813         # env-0 → 8813 | env-1 → 8814 | env-2 → 8815 ...


class SumoEnv(gym.Env):
	def __init__(self, rank = None, render: bool = True, map_config = ["maps/TestMap/osm.sumocfg"], 
		...
		if rank is not None:
			self._env_id = rank # Dùng rank truyền từ SubprocVectorEnv
		else:
			global _env_id_counter
			with _env_id_lock:
				self._env_id = _env_id_counter
				_env_id_counter += 1

		self._label = f"sumo_env_{self._env_id}"
		self._port  = BASE_TRACI_PORT + self._env_id
		self._traci = None   # assigned to a live connection object inside reset()
	...
	# =========================================================================
	# INTERNAL HELPERS
	# All TraCI calls go through self._traci (per-instance connection object)
	# instead of the global traci module.  Logic is 100% unchanged.
	# =========================================================================

	def _veh_exists(self):
		try:
			return self.VEH_ID in self._traci.vehicle.getIDList()
		except Exception:
			return False
	...
	def _get_passenger_edges(self) -> list:
		...
		for edge_id in all_edges:
			...
			try:
				allowed = self._traci.lane.getAllowed(lane_id)
	...
	def _get_surroundings(self):
		...
		my_speed = self._traci.vehicle.getSpeed(self.VEH_ID)
		...
		left_cars = self._traci.vehicle.getNeighbors(self.VEH_ID, 2)
		...
		for n_id, dist in left_cars:
			if dist > 0: # dist > 0 => check front
				if dist < closest_front:
					closest_front = dist
					n_speed = self._traci.vehicle.getSpeed(n_id)
		...
		right_cars = self._traci.vehicle.getNeighbors(self.VEH_ID, 1)
	...
	def _get_obs(self):
		...
		try:
			...
			velocity = self._traci.vehicle.getSpeed(self.VEH_ID) / self.MAX_SPEED
			acceleration = self._traci.vehicle.getAcceleration(self.VEH_ID) / self.MAX_ACCEL
			elec = self._traci.vehicle.getElectricityConsumption(self.VEH_ID) / self.MAX_ELEC
			
			lane_idx = self._traci.vehicle.getLaneIndex(self.VEH_ID)
			road_id = self._traci.vehicle.getRoadID(self.VEH_ID)
			total_lanes = self._traci.edge.getLaneNumber(road_id)
			norm_lane = lane_idx / max(1, total_lanes - 1)

			slope = self._traci.vehicle.getSlope(self.VEH_ID) / self.MAX_SLOPE
			lat_offset = self._traci.vehicle.getLateralLanePosition(self.VEH_ID)
			...
			leader = self._traci.vehicle.getLeader(self.VEH_ID, dist=self.MAX_DIST)
			l_dist, l_rel_speed = (leader[1]/self.MAX_DIST, (velocity - self._traci.vehicle.getSpeed(leader[0]))/self.MAX_SPEED) if leader else (1.0, 0.0)

			# 4. INFRASTRUCTURE (6 biến - giữ nguyên)
			lane_id = self._traci.vehicle.getLaneID(self.VEH_ID)
			speed_limit = self._traci.lane.getMaxSpeed(lane_id) / self.MAX_SPEED if lane_id else 1.0
			can_left = 1.0 if lane_idx < (total_lanes - 1) else 0.0
			can_right = 1.0 if lane_idx > 0 else 0.0
			
			tls_data = self._traci.vehicle.getNextTLS(self.VEH_ID)
			tls_dist, tls_state = (tls_data[0][2]/self.MAX_DIST, (1.0 if tls_data[0][3].lower() == 'g' else 0.0)) if tls_data else (1.0, 1.0)
			
			turn_dist = min(self._traci.lane.getLength(lane_id) - self._traci.vehicle.getLanePosition(self.VEH_ID), self.MAX_DIST) / self.MAX_DIST if lane_id else 1.0
	...
	def _get_dist_to_destination(self):
		try:
			current_edge = self._traci.vehicle.getRoadID(self.VEH_ID)
			...
			if hasattr(self, "current_route_edges") and current_edge in self.current_route_edges:
				...
				for e in remaining_edges:
					dist += self._traci.lane.getLength(f"{e}_0")

				dist -= self._traci.vehicle.getLanePosition(self.VEH_ID)
	...
	def _calculate_reward(self, action):
		...
		cur_speed = self._traci.vehicle.getSpeed(self.VEH_ID)
		...
		elec = self._traci.vehicle.getElectricityConsumption(self.VEH_ID)
		...
		safety_penalty = 0.0
		leader = self._traci.vehicle.getLeader(self.VEH_ID, self.MAX_DIST)
	...
	def reset(self, seed=None, options=None):
		...
		# 1. Close this instance's previous connection (if any).
		#    Closing by label is surgical: it shuts down only the SUMO process
		#    owned by THIS env and leaves all other instances untouched.
		...
		# 3. Start SUMO with a unique label + port for this instance.
		#    traci.start(cmd, label=..., port=...) launches a new SUMO process
		#    and registers it under the given label.  Different labels mean
		#    completely independent connections that never interfere.
		traci.start(SumoCMD, label=self._label, port=self._port)
		...
		# Warmup
		for _ in range(random.randint(300, 600)): self._traci.simulationStep()

		# 4. Setup Vehicle Type
		try:
			existing_types = self._traci.vehicletype.getIDList()
			source_type = "DEFAULT_VEHTYPE"
			if source_type not in existing_types and len(existing_types) > 0:
				source_type = existing_types[0]

			self._traci.vehicletype.copy(source_type, self.VTYPE_ID)
			self._traci.vehicletype.setVehicleClass(self.VTYPE_ID, "passenger")
			self._traci.vehicletype.setColor(self.VTYPE_ID, (0, 255, 0)) 
			self._traci.vehicletype.setParameter(self.VTYPE_ID, "mass", "2911")
			self._traci.vehicletype.setLength(self.VTYPE_ID, "5.1181")
			self._traci.vehicletype.setEmissionClass(self.VTYPE_ID, "MMPEVEM")
			
			# EV Parameters
			self._traci.vehicletype.setParameter(self.VTYPE_ID, "has.battery.device", "true")
			self._traci.vehicletype.setParameter(self.VTYPE_ID, "device.battery.capacity", "123000.00")
			self._traci.vehicletype.setParameter(self.VTYPE_ID, "device.battery.chargeLevel", "123000.00")
			# Hiệu suất thu hồi năng lượng khi phanh (0.0 - 1.0)
			self._traci.vehicletype.setParameter(self.VTYPE_ID, "device.battery.rechargeEfficiency", "0.8")
			# Lực phanh tối đa có thể tái sinh
			self._traci.vehicletype.setParameter(self.VTYPE_ID, "device.battery.maxRegenerationAcceleration", "2.0")
			
			# Imperfection
			for v_type in existing_types:
				self._traci.vehicletype.setParameter(v_type, "sigma", str(self.imperfection))
				self._traci.vehicletype.setParameter(v_type, "impatience", str(self.impatience))
		...
		if self.test_mode:
			...
			try:
				...
				# Add route to SUMO
				self._traci.route.add(route_id, fixed_edges)
				self.current_route_edges = fixed_edges
				
				# Add Vehicle
				self._traci.vehicle.add(self.VEH_ID, route_id, departPos="free", typeID=self.VTYPE_ID)
				
				# Wait for insertion
				for _ in range(50):
					self._traci.simulationStep()
					if self.VEH_ID in self._traci.vehicle.getIDList():
						spawned = True
						break
				
				if spawned:
					# Configure
					self._traci.vehicle.setSpeedMode(self.VEH_ID, 0)
					self._traci.vehicle.setLaneChangeMode(self.VEH_ID, 0)
					
					# Calculate total distance for Reward Normalization
					total_len = 0.0
					for e in fixed_edges:
						try:
							total_len += self._traci.lane.getLength(f"{e}_0")
						except:
							pass
					...
					try: self._traci.vehicle.remove(self.VEH_ID) 
					except: pass
		...
		if self.render_mode and spawned:
			if self.VEH_ID in self._traci.vehicle.getIDList():
				self._traci.gui.trackVehicle("View #0", self.VEH_ID)
				self._traci.gui.setZoom("View #0", 2000)
	...
	def step(self, action):
		...
		# Áp dụng gia tốc
		self._traci.vehicle.setAcceleration(self.VEH_ID, desired_accel, duration=0.5)
		
		# Áp dụng lái (Lane Change) - Logic gọn của Code 1
		LC_THRESHOLD = 0.3
		current_lane_idx = self._traci.vehicle.getLaneIndex(self.VEH_ID)
		if steer_cmd < -LC_THRESHOLD: 
			# Rẽ phải (giảm index) - Cần check max(0, ...)
			self._traci.vehicle.changeLane(self.VEH_ID, max(0, current_lane_idx - 1), 1.0)
			...
			try:
				edge_id = self._traci.vehicle.getRoadID(self.VEH_ID)
				num_lanes = self._traci.edge.getLaneNumber(edge_id)
				self._traci.vehicle.changeLane(self.VEH_ID, min(num_lanes - 1, current_lane_idx + 1), 1.0)
			except: pass
		...
		for _ in range(SIM_STEPS):
			...
			if self.VEH_ID not in self._traci.vehicle.getIDList():
				terminated = True
				# Phân loại lý do chết: Teleport hay Collision
				teleport_list = self._traci.simulation.getStartingTeleportIDList()
			...
			ego_speed = self._traci.vehicle.getSpeed(self.VEH_ID)
			leader = self._traci.vehicle.getLeader(self.VEH_ID, 10.0)
			
			# Check Đèn Đỏ
			next_tls = self._traci.vehicle.getNextTLS(self.VEH_ID)
			is_red_light = False
			...
			# Check Xe Trước Dừng
			is_leader_stopped = (leader is not None and self._traci.vehicle.getSpeed(leader[0]) < 0.5)
			...
			valid_steps += 1
			try:
				e = self._traci.vehicle.getElectricityConsumption(self.VEH_ID)
		...
		try:
			...
			if hasattr(self, "current_route_edges") and self.current_route_edges:
				if current_edge == self.current_route_edges[-1]:
					...
					lane_len = self._traci.lane.getLength(self._traci.vehicle.getLaneID(self.VEH_ID))
					pos = self._traci.vehicle.getLanePosition(self.VEH_ID)
	...
	def close(self):
		# Close only this instance's own labeled connection.
		# Other instances are completely unaffected.
		if self._traci is not None:
			try:
				self._traci.close()
			except:
